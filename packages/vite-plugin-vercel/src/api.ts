...
import type { PluginContext, ViteVercelConfig } from "./types";
...
export function getVercelAPI(pluginContextOrServer: Pick<PluginContext, "environment"> | ViteDevServer) {
  const config =
    "environment" in pluginContextOrServer ? pluginContextOrServer.environment.config : pluginContextOrServer.config;
  const vpv: Plugin<(pluginContext?: Pick<PluginContext, "environment">) => ViteVercelApi> | undefined =
    config.plugins.find((p) => p.name === "vite-plugin-vercel:api");
